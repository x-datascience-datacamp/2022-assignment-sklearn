...
class KNearestNeighbors(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        # For each point of X, check the k nearest neighbors
        for i, x in enumerate(X):
            # Compute the distance between x and all the training points
            dist = pairwise_distances(x.reshape(1, -1),
                                      self.X_train_,
                                      metric='euclidean')
            # Get the k nearest neighbors
            neighbors = np.argsort(dist)[0, :self.n_neighbors]
            # Get the most frequent class among the k nearest neighbors
    ...
    def score(self, X, y):
        ...
        check_is_fitted(self)
        X, y = check_X_y(X, y)
        # Apply fit to each and compare with target values
        y_pred = self.predict(X)
        # Compute the accuracy between y_pred and y
...
class MonthlySplit(BaseCrossValidator):
    ...
    def get_n_splits(self, X, y=None, groups=None):
        ...
        # Checks
        X = X.reset_index()
        # Check that the time_col is of type datetime
    ...
    def split(self, X, y, groups=None):
        ...
        # Resample X based on month & Get the indexes for each month
        X = (X.reset_index()
             .resample('M', on=self.time_col)
             .apply(lambda x: x.index))
        # Yield the train and test indexes for each split
