...
class KNearestNeighbors(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        y_neighbors = self.y_train_[index]
...
class MonthlySplit(BaseCrossValidator):
    ...
    def split(self, X, y, groups=None):
        ...
        for i in range(n_splits):
            idx_train = X.index[X_t.dt.to_period('M') ==
                                np.unique(X_t.dt.to_period('M'))[i]].values
            idx_test = X.index[X_t.dt.to_period('M') ==
                               np.unique(X_t.dt.to_period('M'))[i+1]].values
            yield (
                idx_train, idx_test
            )
